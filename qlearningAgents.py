...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        if len(actions) != 0:
            actionVal = []
            for action in actions:
                actionVal.append(self.getQValue(state, action))
            return actions[actionVal.index(max(actionVal))]
        else:
            return None
        # util.raiseNotDefined()
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        qVal = 0
        for each in feature:
            qVal += feature[each] * weights[each]
        return qVal
        # util.raiseNotDefined()
    ...
    def update(self, state, action, nextState, reward):
        ...
        for each in feature:
            self.weights[each] = self.weights[each] + self.alpha * feature[each] * diff
        # util.raiseNotDefined()
